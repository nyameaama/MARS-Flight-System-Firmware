...
minmea_rescale(const struct minmea_float* f, int_least32_t new_scale)
{
    if (f->scale == 0)
        return 0;
 ...
 */
static inline float
minmea_tofloat(const struct minmea_float* f)
{
    if (f->scale == 0)
        return NAN;
 ...
 */
static inline float
minmea_tocoord(const struct minmea_float* f)
{
    if (f->scale == 0)
        return NAN;
