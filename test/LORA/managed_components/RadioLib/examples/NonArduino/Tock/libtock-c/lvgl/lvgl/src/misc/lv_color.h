...
/**
 * MSVC compiler's definition of the __cplusplus indicating 199711L regardless to C++ standard
 * version see https://devblogs.microsoft.com/cppblog/msvc-now-correctly-reports-cplusplus so we use
 * _MSC_VER macro instead of __cplusplus
 */
#ifdef _MSC_VER
...
typedef lv_color_t (*lv_color_filter_cb_t)(const struct _lv_color_filter_dsc_t*, lv_color_t,
                                           lv_opa_t);
...
lv_color_mix(lv_color_t c1, lv_color_t c2, uint8_t mix)
{
    ...
    LV_COLOR_SET_R(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_R(c1) * mix +
                                   LV_COLOR_GET_R(c2) * (255 - mix) + LV_COLOR_MIX_ROUND_OFS));
    LV_COLOR_SET_G(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_G(c1) * mix +
                                   LV_COLOR_GET_G(c2) * (255 - mix) + LV_COLOR_MIX_ROUND_OFS));
    LV_COLOR_SET_B(ret, LV_UDIV255((uint16_t)LV_COLOR_GET_B(c1) * mix +
                                   LV_COLOR_GET_B(c2) * (255 - mix) + LV_COLOR_MIX_ROUND_OFS));
...
/**
 * Mix two colors with a given ratio. It runs faster then `lv_color_mix` but requires some pre
 * computation.
...
lv_color_mix_premult(uint16_t* premult_c1, lv_color_t c2, uint8_t mix)
{
    ...
    /*LV_COLOR_DEPTH == 8, 16 or 32*/
    LV_COLOR_SET_R(ret,
                   LV_UDIV255(premult_c1[0] + LV_COLOR_GET_R(c2) * mix + LV_COLOR_MIX_ROUND_OFS));
    LV_COLOR_SET_G(ret,
                   LV_UDIV255(premult_c1[1] + LV_COLOR_GET_G(c2) * mix + LV_COLOR_MIX_ROUND_OFS));
    LV_COLOR_SET_B(ret,
                   LV_UDIV255(premult_c1[2] + LV_COLOR_GET_B(c2) * mix + LV_COLOR_MIX_ROUND_OFS));
    LV_COLOR_SET_A(ret, 0xFF);
#else
    /*LV_COLOR_DEPTH == 1*/
    /*Restore color1*/
 ...
 */
LV_ATTRIBUTE_FAST_MEM static inline void
lv_color_mix_with_alpha(lv_color_t bg_color, lv_opa_t bg_opa, lv_color_t fg_color, lv_opa_t fg_opa,
 ...
 */
static inline uint8_t
lv_color_brightness(lv_color_t color)
{
    lv_color32_t c32;
    c32.full = lv_color_to32(color);
    uint16_t bright =
        (uint16_t)(3u * LV_COLOR_GET_R32(c32) + LV_COLOR_GET_B32(c32) + 4u * LV_COLOR_GET_G32(c32));
...
/**
 * Just a wrapper around LV_COLOR_CHROMA_KEY because it might be more convenient to use a function
 * is some cases
 * @return LV_COLOR_CHROMA_KEY
 */
static inline lv_color_t
lv_color_chroma_key(void)
{
    return LV_COLOR_CHROMA_KEY;
}
