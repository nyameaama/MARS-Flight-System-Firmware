...
typedef enum
{
    ...
    LV_EVENT_PRESS_LOST,    /**< The object is still being pressed but slid cursor/finger off of the
                               object */
    LV_EVENT_SHORT_CLICKED, /**< The object was pressed for a short period of time, then released
                               it. Not called if scrolled.*/
    LV_EVENT_LONG_PRESSED,  /**< Object has been pressed for at least `long_press_time`.  Not called
                               if scrolled.*/
    ...
    LV_EVENT_REFR_EXT_DRAW_SIZE, /**< Get the required extra draw area around the object (e.g. for
                                    shadow). The event parameter is `lv_coord_t *` to store the
                                    size.*/
    ...
    LV_EVENT_INSERT,        /**< A text is inserted to the object. The event data is `char *` being
                               inserted.*/
...
/**
 * Used as the event parameter of ::LV_EVENT_HIT_TEST to check if an `point` can click the object or
 * not. `res` should be set like this:
 *   - If already set to `false` an other event wants that point non clickable. If you want to
 * respect it leave it as `false` or set `true` to overwrite it.
...
{
    const lv_point_t*
        point; /**< A point relative to screen to check if it can click the object or not*/
    bool res;  /**< true: `point` can click the object; false: it cannot*/
} lv_hit_test_info_t;

/**
 * Used as the event parameter of ::LV_EVENT_COVER_CHECK to check if an area is covered by the
 * object or not. In the event use `const lv_area_t * area = lv_event_get_cover_area(e)` to get the
 * area to check and `lv_event_set_cover_res(e, res)` to set the result.
 */
typedef struct
{
    lv_cover_res_t res;
 ...
 * @return          LV_RES_OK: the taget object was not deleted in the event; LV_RES_INV: it was
 ...
 * Mark this object's `event_temp_data` deleted to know that it's `lv_event_send` should return
 * `LV_RES_INV`
 ...
 * An object can have multiple event handler. They will be called in the same order as they were
 * added.
 * @param obj       pointer to an object
 * @param filter    and event code (e.g. `LV_EVENT_CLICKED`) on which the event should be called.
 * `LV_EVENT_ALL` can be sued the receive all the events.
 * @param event_cb  the new event function
 * @param           user_data custom data data will be available in `event_cb`
 * @return          a pointer the event descriptor. Can be used in ::lv_obj_remove_event_dsc
 */
struct _lv_event_dsc_t* lv_obj_add_event_cb(struct _lv_obj_t* obj, lv_event_cb_t event_cb,
...
/**
 * Get a pointer to an `lv_point_t` variable in which the self size should be saved (width in
 * `point->x` and height `point->y`). Can be used in `LV_EVENT_GET_SELF_SIZE`
 * @param e     pointer to an event
...
/**
 * Get a pointer to an `lv_hit_test_info_t` variable in which the hit test result should be saved.
