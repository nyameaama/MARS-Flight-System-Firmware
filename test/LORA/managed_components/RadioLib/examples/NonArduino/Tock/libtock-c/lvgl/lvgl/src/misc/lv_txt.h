...
enum
{
    ...
    LV_TEXT_FLAG_FIT =
        0x04, /**< Max-width is already equal to the longest line. (Used to skip some calculation)*/
};
 ...
 */
lv_coord_t lv_txt_get_width(const char* txt, uint32_t length, const lv_font_t* font,
 ...
 */
void _lv_txt_encoded_letter_next_2(const char* txt, uint32_t* letter, uint32_t* letter_next,
                                   uint32_t* ofs);
 ...
 */
static inline bool
_lv_txt_is_break_char(uint32_t letter)
{
    uint8_t i;
    bool ret = false;
