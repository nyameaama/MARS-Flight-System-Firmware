...
#define LV_COORD_MAX \
    ((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - _LV_COORD_MAX_REDUCE))
 ...
 */
inline static void
lv_area_copy(lv_area_t* dest, const lv_area_t* src)
{
    dest->x1 = src->x1;
    dest->y1 = src->y1;
    dest->x2 = src->x2;
...
lv_area_get_width(const lv_area_t* area_p)
{
 ...
 */
static inline lv_coord_t
lv_area_get_height(const lv_area_t* area_p)
{
 ...
 */
void lv_area_align(const lv_area_t* base, lv_area_t* to_align, lv_align_t align, lv_coord_t ofs_x,
                   lv_coord_t ofs_y);
