...
enum
{
    ...
    LV_PART_INDICATOR =
        0x020000, /**< Indicator, e.g. for slider, bar, switch, or the tick box of the checkbox*/
...
enum
{
    ...
    LV_OBJ_FLAG_ADV_HITTEST = (1 << 14), /**< Allow performing more accurate hit (click) test. E.g.
 ...
 * If specified in the styles, transition animation will be started from the previous state to the
 * current.
 * @param obj       pointer to an object
...
#if LV_USE_USER_DATA
static inline void
lv_obj_set_user_data(lv_obj_t* obj, void* user_data)
{
...
lv_obj_dpx(const lv_obj_t* obj, lv_coord_t n)
{
