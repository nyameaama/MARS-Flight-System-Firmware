...
typedef enum
{
    ...
    GAP_INCOMPLETE_LIST_32BIT_SERVICE_IDS = 0x04, /* Incomplete list of 32-bit Service IDs (not
                                                     relevant for Bluetooth 4.0). */
    GAP_COMPLETE_LIST_32BIT_SERVICE_IDS = 0x05, /* Complete list of 32-bit Service IDs (not relevant
                                                   for Bluetooth 4.0). */
...
//
// Returns the number of bytes written to the `adv_data` buffer.
int gap_add_adv_data_field(AdvData_t* adv_data, GapAdvertisementData_t type, const uint8_t* data,
                           uint8_t data_len);
