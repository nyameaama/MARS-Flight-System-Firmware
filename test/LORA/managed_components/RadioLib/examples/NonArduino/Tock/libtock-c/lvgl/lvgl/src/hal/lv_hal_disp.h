...
typedef struct _lv_disp_draw_buf_t
{
    ...
    /*1: It was the last chunk to flush. (It can't be a bit field because when it's cleared from IRQ
     * Read-Modify-Write issue might occur)*/
...
typedef struct _lv_disp_drv_t
{
    ...
    uint32_t rotated : 2;       /**< 1: turn the display by 90 degree. @warning Does not update
                                   coordinates for you!*/
    uint32_t screen_transp : 1; /**Handle if the screen doesn't have a solid (opa == LV_OPA_COVER)
    ...
    /** MANDATORY: Write the internal buffer (draw_buf) to the display. 'lv_disp_flush_ready()' has
    ...
    void (*set_px_cb)(struct _lv_disp_drv_t* disp_drv, uint8_t* buf, lv_coord_t buf_w, lv_coord_t x,
    ...
    void (*gpu_fill_cb)(struct _lv_disp_drv_t* disp_drv, lv_color_t* dest_buf,
...
typedef struct _lv_disp_t
{
    ...
    uint32_t screen_cnt;
    uint8_t del_prev : 1; /**< 1: Automatically delete the previous screen when the screen load
                             animation is ready*/
 ...
 * Can be called from `flush_cb` to execute some special display refreshing if needed when all areas
 * area flushed.
 * @param disp_drv pointer to display driver
 * @return true: it's the last area to flush; false: there are other areas too which will be
 * refreshed soon
 */
