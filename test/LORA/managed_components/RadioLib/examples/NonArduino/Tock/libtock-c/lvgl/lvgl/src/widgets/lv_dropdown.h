 ...
 * The options will be copied and saved in the object so the `options` can be destroyed after
...
/**
 * Set the options in a drop-down list from a static string (global, static or dynamically
 * allocated). Only the pointer of the option string will be saved.
 * @param obj       pointer to drop-down list object
...
/**
 * Set an arrow or other symbol to display when on drop-down list's button.  Typically a down caret
 * or arrow.
 * @param obj       pointer to drop-down list object
 * @param symbol    a text like `LV_SYMBOL_DOWN`, an image (pointer or path) or NULL to not draw
 * symbol icon
