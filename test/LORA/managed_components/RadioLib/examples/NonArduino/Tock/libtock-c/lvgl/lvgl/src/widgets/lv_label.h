...
enum
{
    LV_LABEL_LONG_WRAP,   /**< Keep the object width, wrap the too long lines and expand the object
                             height*/
...
typedef struct
{
    ...
    uint8_t dot_tmp_alloc : 1; /*1: dot_tmp has been allocated;.0: dot_tmp directly holds up to 4
                                  bytes of characters*/
} lv_label_t;
 ...
 * @param index     character index from where selection should start. `LV_LABEL_TEXT_SELECTION_OFF`
 * for no selection
 */
 ...
 * @param obj       pointer to a label object
 * @param index     character index where selection should end.  `LV_LABEL_TEXT_SELECTION_OFF` for
 * no selection
 */
 ...
 * @param pos       store the result here (E.g. index = 0 gives 0;0 coordinates if the text if
 * aligned to the left)
 */
 ...
 * @return          The index of the letter on the 'pos_p' point (E.g. on 0;0 is the 0. letter if
 * aligned to the left) Expressed in character index and not byte index (different in UTF-8)
 */
 ...
 * @param label     pointer to a label object
 * @param pos       character index from where to cut. Expressed in character index and not byte
 * index. 0: start in from of the first character
