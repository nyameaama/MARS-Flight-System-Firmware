 ...
 * @param obj       pointer to an object to align
 * @param base      pointer to an other object (if NULL `obj`s parent is used). 'obj' will be
 * aligned to it.
 * @param align     type of alignment (see 'lv_align_t' enum)
 * @param x_ofs     x coordinate offset after alignment
 * @param y_ofs     y coordinate offset after alignment
 * @note            if the position or size of `base` changes `obj` needs to be aligned manually
 * again
 */
void lv_obj_align_to(struct _lv_obj_t* obj, const struct _lv_obj_t* base, lv_align_t align,
 ...
 */
static inline void
lv_obj_center(struct _lv_obj_t* obj)
{
 ...
 * @param obj       pointer to an object
 * @return          distance of `obj` from the left side of its parent plus the parent's left
 * padding
 * @note            Zero return value means the object is on the left padding of the parent, and not
 * on the left edge.
 * @note            Scrolling of the parent doesn't change the returned value.
 * @note            The returned value is always the distance from the parent even if `obj` is
 ...
 * @param obj       pointer to an object
 * @return          distance of `obj` from the right side of its parent plus the parent's right
 * padding
 * @note            Zero return value means the object is on the right padding of the parent, and
 ...
 * @note            Zero return value means the object is on the top padding of the parent, and not
 * on the top edge.
 * @note            Scrolling of the parent doesn't change the returned value.
 * @note            The returned value is always the distance from the parent even if `obj` is
 * positioned by a layout.
 ...
 * @param obj       pointer to an object
 * @return          distance of `obj` from the bottom side of its parent plus the parent's bottom
 * padding
 * @note            Zero return value means the object is on the bottom padding of the parent, and
 ...
 * @param obj       pointer to an object
 * @return          the width which still fits into its parent without causing overflow (making the
 * parent scrollable)
 */
 ...
 * @param obj       pointer to an object
 * @return          the height which still fits into the parent without causing overflow (making the
 * parent scrollable)
 */
 ...
 * @param obj       pointer to an object
 * @param area      the area which still fits into the parent without causing overflow (making the
 * parent scrollable)
 */
...
void lv_obj_move_children_by(struct _lv_obj_t* obj, lv_coord_t x_diff, lv_coord_t y_diff,
...
/**
 * Clamp a width between min and max width. If the min/max width is in percentage value use the
 * ref_width
 ...
 */
lv_coord_t lv_clamp_width(lv_coord_t width, lv_coord_t min_width, lv_coord_t max_width,
                          lv_coord_t ref_width);

/**
 * Clamp a height between min and max height. If the min/max height is in percentage value use the
 * ref_height
 ...
 */
lv_coord_t lv_clamp_height(lv_coord_t height, lv_coord_t min_height, lv_coord_t max_height,
                           lv_coord_t ref_height);
