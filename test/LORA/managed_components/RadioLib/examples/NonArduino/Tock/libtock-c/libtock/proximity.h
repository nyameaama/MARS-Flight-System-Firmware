...
// A proximity value is of the following range [0,255] where '255' indicates the closest measurable
// distance and '0' that no object is detected.
//
...
// This function can be used to wait for the sensor to detect a proximity reading in the
// user-specified range. This range is determined by the `proximity_set_interrupt_thresholds`
// arguments.
...
// This function can be used to wait for the sensor to detect a proximity reading in the
// user-specified range. This range is determined by the `proximity_set_interrupt_thresholds`
// arguments. A proximity value is of the following range [0,255] where '255' indicates the closest
// measurable distance and '0' that no object is detected.
//
// proximity      - pointer to byte-sized proximity data returned from the sensor.
...
//
// The proximity sensor fires an interrupt (which executes the callback sent by the
// `proximity_read_on_interrupt_sync()` function) The interrupt is fired when the following logic
// statement is true `(reading >= upper_threshold) || (reading <= lower_threshold)`
//
