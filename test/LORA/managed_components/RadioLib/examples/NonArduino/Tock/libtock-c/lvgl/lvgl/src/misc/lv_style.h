...
#define LV_STYLE_PROP_PARENT_LAYOUT_REFR                                       \
    (1 << 13)                          /*Requires layout update on parent when \
                                          changed*/
 ...
 *         LV_RES_OK: the property was fond, and `value` is set accordingly
 * @note For performance reasons there are no sanity check on `style`
 * @note This function is the same as ::lv_style_get_prop but inlined. Use it only on performance
 * critical places
 */
static inline lv_res_t
lv_style_get_prop_inlined(lv_style_t* style, lv_style_prop_t prop, lv_style_value_t* value)
{
    if (style->is_const)
    {
        const lv_style_const_prop_t* const_prop;
        for (const_prop = style->v_p.const_props; const_prop->prop != LV_STYLE_PROP_INV;
             const_prop++)
        {
 ...
 * @param user_data any custom data that will be saved in the transition animation and will be
 * available when `path_cb` is called
 * @example
...
void lv_style_transition_dsc_init(lv_style_transition_dsc_t* tr, const lv_style_prop_t props[],
                                  lv_anim_path_cb_t path_cb, uint32_t time, uint32_t delay,
                                  void* user_data);
...
/**
 * Tell the group of a property. If the a property from a group is set in a style the (1 << group)
 * bit of style->has_group is set. It allows early skipping the style if the property is not exists
 * in the style at all.
