...
typedef struct
{
    ...
    uint32_t adv_w : 12; /**< Draw the next glyph after this width. 8.4 format (real_value * 16 is
                            stored).*/
    ...
    uint32_t
        adv_w; /**< Draw the next glyph after this width. 28.4 format (real_value * 16 is stored).*/
...
typedef struct
{
    ...
    const uint8_t*
        left_class_mapping; /*Map the glyph_ids to classes: index -> glyph_id -> class_id*/
    const uint8_t*
        right_class_mapping; /*Map the glyph_ids to classes: index -> glyph_id -> class_id*/
...
/**
 * Used as `get_glyph_bitmap` callback in LittelvGL's native font format if the font is
 * uncompressed.
