 ...
 * Every write to an SPI register using SPI set function will be verified by a subsequent read
 * operation. This improves reliability, but slightly slows down communication. Note: Enabled by
 * default.
 */
 ...
 * RadioLib will check provided parameters (such as frequency) against limits determined by the
 * device manufacturer. It is highly advised to keep this macro defined, removing it will allow
 * invalid values to be set, possibly leading to bricked module and/or program crashing. Note:
 * Enabled by default.
 */
 ...
 * Warning: SX127x errata fix has been reported to cause issues with LoRa bandwidths lower than 62.5
 * kHz. It should only be enabled if you really are observing some errata-related issue. Note:
 * Disabled by default.
 */
...
/*
 * God mode enable - all methods and member variables in all classes will be made public, thus
 * making them accessible from Arduino code. Warning: Come on, it's called GOD mode - obviously only
 * use this if you know what you're doing. Failure to heed the above warning may result in bricked
 * module.
 */
 ...
 * This will make some hardware methods like SPI get/set accessible from the user sketch - think of
 * it as "god mode lite" Warning: RadioLib won't stop you from writing invalid stuff into your
 * device, so it's quite easy to brick your module with this.
 */
 ...
 * Warning: Large static arrays will be created in some methods. It is not advised to send large
 ...
 *
 * RADIOLIB_PLATFORM - platform name, used in debugging to quickly check the correct platform is
 * detected. RADIOLIB_NC - alias for unused pin, usually the largest possible value of uint8_t.
 ...
 * (usually Flash). RADIOLIB_TYPE_ALIAS - construct to create an alias for a type, usually vai the
 * `using` keyword. RADIOLIB_TONE_UNSUPPORTED - some platforms do not have tone()/noTone(), which is
 * required for AFSK. RADIOLIB_BUILTIN_MODULE - some platforms have a builtin radio module on fixed
 * pins, this macro is used to specify that pinout.
 *
...
// some of RadioLib drivers may be excluded, to prevent collisions with platforms (or to speed up
// build process) the following is a complete list of all possible exclusion macros, uncomment any
// of them to disable that driver NOTE: Some of the exclusion macros are dependent on each other.
// For example, it is not possible to exclude RF69
//       while keeping SX1231 (because RF69 is the base class for SX1231). The dependency is always
//       uni-directional, so excluding SX1231 and keeping RF69 is valid.
...
#elif defined(__AVR__) && !(defined(ARDUINO_AVR_UNO_WIFI_REV2) || \
                            defined(ARDUINO_AVR_NANO_EVERY) || defined(ARDUINO_ARCH_MEGAAVR))
...
// Arduino mbed OS boards have a really bad tone implementation which will crash after a couple
// seconds
#define RADIOLIB_TONE_UNSUPPORTED
...
// ... and also defines pinMode() as a macro, which is by far the stupidest thing I have seen on
// Arduino
#undef pinMode
...
// ... AND as the (hopefully) final nail in the coffin, IT F*CKING DEFINES YIELD() AS A MACRO THAT
// DOES NOTHING!!!
#define RADIOLIB_YIELD_UNSUPPORTED
#if defined(yield)
#undef yield
...
/*!
  \brief Macro to check variable is within constraints - this is commonly used to check parameter
  ranges. Requires RADIOLIB_CHECK_RANGE to be enabled
*/
...
// these macros are usually defined by Arduino, but some platforms undef them, so its safer to use
// our own
